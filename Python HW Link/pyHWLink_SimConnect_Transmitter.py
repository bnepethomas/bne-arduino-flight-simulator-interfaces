...
##    print("GEAR_RIGHT_POSITION is: " + str(GEAR_RIGHT_POSITION.value))
    ...
    if last_center_gear_pos != GEAR_CENTER_POSITION.value :
        ...
        if last_center_gear_pos >= 0.98 :
            Set_Nose_Gear_Led(1)
        else :
            Set_Nose_Gear_Led(0)

    if last_left_gear_pos != GEAR_LEFT_POSITION.value :
        last_left_gear_pos = GEAR_LEFT_POSITION.value
        if last_left_gear_pos >= 0.98 :
            Set_Left_Gear_Led(1)
        else :
            Set_Left_Gear_Led(0)
            
    if last_right_gear_pos != GEAR_RIGHT_POSITION.value :
        last_right_gear_pos = GEAR_RIGHT_POSITION.value
        if last_right_gear_pos >= 0.98 :
            Set_Right_Gear_Led(1)
        else :
...
def Update_Flaps():
    ...
    global last_flaps_pos
    global Radians

##    print("TRAILING_EDGE_FLAPS_LEFT_ANGLE is: " + str(TRAILING_EDGE_FLAPS_LEFT_ANGLE.value * Radians))


    if last_flaps_pos != TRAILING_EDGE_FLAPS_LEFT_ANGLE.value :
        last_flaps_pos = TRAILING_EDGE_FLAPS_LEFT_ANGLE.value

        if last_flaps_pos * Radians  < 10 :
            ##print("Flaps up")
            Set_Half_Flaps_Led(0)
            Set_Full_Flaps_Led(0)
        elif (last_flaps_pos * Radians) > 20:
            ## print("Flaps full")
            Set_Half_Flaps_Led(0)
            Set_Full_Flaps_Led(1)
        else :
            ## print("Flaps half")
...
def StartSimConnect():
    ...
    try: 
        ...
        print("Grabbing TRAILING_EDGE_FLAPS_LEFT_ANGLE")
        TRAILING_EDGE_FLAPS_LEFT_ANGLE = aq.find("TRAILING_EDGE_FLAPS_LEFT_ANGLE")
