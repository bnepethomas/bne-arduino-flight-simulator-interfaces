...
|---|---|---|
| `FuelLoadStepper` (was `RadarAltStepper`) | FULL4WIRE, `STEPPER_FL_COIL_A..D` (32/33/34/35, wired C,D,A,B — same physical pins the original sketch used for Radar Alt) | Real PSI now via `setFuelLoad()`/`FUEL_LOAD_PSI_TABLE` (`FUELLOAD` code) — 2-point bench-measured table so far (0 PSI→0 steps, 30 PSI→160 steps), linear interpolation between them, clamped (not extrapolated) above 30 PSI until a higher point is measured. `FUELLOADRAW` added as the raw-step bypass, matching every other calibrated gauge. Boot startup swing active (`SwingFUELLOAD` = true) — still sweeps the full `X27_FULLWIRE_STEPS` range for a self-test, not clamped to the calibrated 0-160 range |
| `ElectricalLoadStepper` (was `ETstepper`) | FULL4WIRE, `EL_COIL_A..D` (36/37/38/39 — same physical pins the original sketch used for Torque) | Real percent now via `setElectricalLoad()`/`ELECTRICAL_LOAD_PCT_TABLE` (`ELECTRICALLOAD` code) — 2-point bench-measured table (0%→22 steps, 100%→200 steps; unlike every other table here, 0% is NOT the stepper's raw zero). `ELECTRICALLOADRAW` added as the raw-step bypass. Boot startup swing active, gated by `SwingELECTRICALLOAD` (`true`) — that define existed already but had no matching startup block until now; still sweeps the full `X27_FULLWIRE_STEPS` range for a self-test, not clamped to the calibrated 22-200 range |
...
| `EOTstepper` (`OILT`, Engine Oil Temp) | Unchanged pins/interface | Real degrees C now via `setEOT()`/`EOT_C_TABLE` (4-point bench-measured table: 0C→0, 50C→79, 100C→156, 150C→231 steps; replaces the old placeholder linear scale it shared with EGT/XOT/XOP/N1/FUEL). Boot startup swing active (`SwingEOT` = true) - uses `FULL4WIRE_STEPS`/`FULL4WIRE_HOMING_STEPS` (315/316), not the X27 constants Fuel Load/Electrical Load/TS/RS use; overshoots the real 231-step calibrated max for a fuller self-test, same as Fuel Load's swing does. Returns to raw 0 (`EOT_C_TABLE`'s 0C row is already 0 steps, no fine-trim offset needed) |
| `EOPstepper` (`OILP`, Engine Oil Pressure) | Unchanged pins/interface | Real PSI now via `setEOP()`/`EOP_PSI_TABLE` (4-point bench-measured table: 0→0, 50→74, 100→148, 150→220 steps; replaces the old placeholder linear scale, reuses the `PsiToStepEntry` struct `FUEL_LOAD_PSI_TABLE` also uses). Boot startup swing active (`SwingEOP` = true) - same `FULL4WIRE_STEPS`/`FULL4WIRE_HOMING_STEPS` range as `EOTstepper` above, overshoots the real 220-step calibrated max for a fuller self-test. Returns to raw 0 |
